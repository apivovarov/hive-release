...
###############################################################################
###
### Installs Hcatalog component.
###
### Arguments:
###     component: Component to be installed, it should be hcatalog
    ...
    )
{
    if ( $component -eq "hcatalog" )
    {
        ...
        $hcatInstallPath = Join-Path $nodeInstallRoot $FinalName

        Write-Log "Installing Apache Hcatalog @final.name@ to $hcatInstallPath"
        ...
        ###
        Write-Log "Extracting $FinalName.zip to $hcatInstallPath"
        Write-Log "Setting TEMPLETON_HOME to $installToDir at machine scope"
...
###############################################################################
###
### Alters the configuration of the hcatalog component.
###
### Arguments:
###     component: Component to be configured, it should be "Hcatalog"
###     nodeInstallRoot: Target install folder (for example "C:\Hadoop")
###     serviceCredential: Credential object used for service creation
###     configs: Configuration that should be applied.
###              For example, @{"templeton.jar" = "C:\Hadoop\Templeton\templeton.jar"}
    ...
    $configs)
{
    ...
    if ( $component -eq "templeton" )
    {
        foreach( $key in empty-null $configs.Keys )
        {
            ### No aliases supported at the moment, so just add to the list
            {
