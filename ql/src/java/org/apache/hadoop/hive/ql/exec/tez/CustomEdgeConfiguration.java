...
class CustomEdgeConfiguration implements Writable {
  ...
  public CustomEdgeConfiguration(int numBuckets, Multimap<Integer, Integer> routingTable) {
    ...
    if (routingTable != null) {
      initType = InitType.VERTEX_INITIALIZED;
    } else {
      this.initType = InitType.EDGE_INITIALIZED;
    }
  }
  
  @Override
  public void write(DataOutput out) throws IOException {
    out.writeInt(initType.ordinal());
  ...
  public void readFields(DataInput in) throws IOException {
    ...
    this.initType = InitType.values()[proto];
    this.numBuckets = in.readInt();
    if (this.initType == InitType.EDGE_INITIALIZED) {
      return;
    }
    ...
    this.numBuckets = bucketToTaskMap.keySet().size();
  }
