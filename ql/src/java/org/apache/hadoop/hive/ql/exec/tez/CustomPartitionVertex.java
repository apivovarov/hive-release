...
public class CustomPartitionVertex implements VertexManagerPlugin {
  ...
  @Override
    public void onRootVertexInitialized(String inputName, InputDescriptor inputDescriptor,
      ...
      // TODO Hive: Ideally, since there's only 1 Input expected at the moment -
      // ensure this method is called onlu once. Tez will ccall it once per Root
      // Input.

      try {
        // This is using the payload from the RootVertexInitializer corresponding
        // to InputName.
        // Ideally it should be using it's own configuration claass - but that
        // means serializing another instance. TODO efficiently eventually
        ...
        // HACK: FIXME : This assumes that Grouping will always be used. Changing the InputFormat - so that the correct one is initialized in MRInput.
      ...
      try {
        groupSplits(bucketToInitialSplitMap);
  ...
  private void processAllEvents(String inputName) throws IOException {
    ...
    int taskCount = 0;
    for (Entry<Integer, Collection<InputSplit>> entry : bucketToInitialSplitMap.asMap().entrySet()) {
    ...
    // TODO Avoid all this if Grouping is not used.
    int count = 0;
  ...
  private void groupSplits (Multimap<Integer, InputSplit> bucketSplitMultiMap) throws IOException {
    Multimap<Integer, InputSplit>clone = 
      ArrayListMultimap.<Integer, InputSplit>create(bucketSplitMultiMap);
    Map<Integer, Collection<InputSplit>> bucketSplitMap = clone.asMap();
    ...
    for (int bucketId : bucketSplitMap.keySet()) {
      ...
      InputSplit[] groupedSplits = grouper.getGroupedSplits(conf, 
          inputSplitCollection.toArray(new InputSplit[0]), 0, 
      ...
      bucketSplitMultiMap.removeAll(bucketId);
      for (InputSplit inSplit : groupedSplits) {
        bucketSplitMultiMap.put(bucketId, inSplit);
      }
    }
  }
