...
public class TezTask extends Task<TezWork> {
  ...
  public int execute(DriverContext driverContext) {
    ...
    try {
      ...
      boolean canReloc = HiveConf.getBoolVar(conf, HiveConf.ConfVars.TEZ_RELOCALIZE_JARS_FOR_AM);
      ...
      if (!session.isOpen()) {
        ...
        session.localizeAdditionalFiles(DagUtils.getTempFilesFromConf(ss
            .getConf()));
...
// TODO#: here
  ...
  DAGClient submit(JobConf conf, DAG dag, Path scratchDir,
      LocalResource appJarLr, List<LocalResource> addedLr, TezSessionState sessionState)
    ...
    Map<String, LocalResource> additionalAmResources = null;
    if (addedLr != null && !addedLr.isEmpty()) {
      additionalAmResources = new HashMap<String, LocalResource>();
      for (LocalResource lr : addedLr) {
        additionalAmResources.put(utils.getBaseName(lr), lr);
      }
    }
